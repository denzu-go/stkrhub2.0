...
$sqlUniqueOrderDates = "SELECT DISTINCT unique_order_group_id FROM orders WHERE to_deliver = 1 AND user_id = $user_id ORDER BY order_date DESC";
...
while ($row = $queryUniqueOrderDates->fetch_assoc()) {
    ...
    $sqlOrderDetails = "SELECT * FROM orders WHERE unique_order_group_id = $unique_order_group_id AND to_deliver = 1 AND user_id = $user_id";
    ...
    $subtotal = 0;
    $sqlAll = "SELECT * FROM orders WHERE unique_order_group_id = $unique_order_group_id AND to_deliver = 1 AND user_id = $user_id AND ticket_id IS NULL";
